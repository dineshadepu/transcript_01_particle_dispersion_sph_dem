...
class Case0(Application):
    ...
    def consume_user_options(self):
        ...
        if self.stirrer_velocity > 1e-12:
    ...
    def create_particles(self):
        ...
        stirrer_dem.x[:] -= self.body_diameter * 1.
        stirrer_dem.x[:] -= self.body_diameter * 0.5
        # stirrer_dem.x[:] = self.body_diameter * 20.
        stirrer_dem.y[:] -= min(stirrer_dem.y) - min(wall.y)
        stirrer_dem.y[:] -= max(stirrer_dem.y) - min(body_slave.y)
        stirrer_dem.y[:] -= self.body_diameter * 1.
